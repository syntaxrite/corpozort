  ...
  return (
    ...
    >
      <div className="w-16 h-16 relative">
        {/* Lemming body */}
        <div className="w-full h-full bg-amber-700 rounded-full relative shadow-lg">
          {/* Head */}
          <div className="absolute top-1 left-1/2 -translate-x-1/2 w-6 h-6 bg-amber-600 rounded-full"></div>
          ...
          {/* Snout */}
          <div className="absolute top-3.5 left-1/2 -translate-x-1/2 w-2 h-1.5 bg-amber-500 rounded-full"></div>
          
          {/* Front paws */}
          <div className="absolute top-4 left-2 w-1.5 h-3 bg-amber-800 rounded-full"></div>
          <div className="absolute top-4 right-2 w-1.5 h-3 bg-amber-800 rounded-full"></div>
          
          {/* Tail */}
          <div className="absolute bottom-0 right-1 w-2 h-4 bg-amber-800 rounded-full opacity-70"></div>
        </div>

        {/* Thought bubble */}
        <motion.div
          className="absolute -top-8 left-1/2 -translate-x-1/2 bg-white/90 text-amber-900 px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap shadow-md"
...
const colorClasses = {
  amber: { badge: "bg-amber-600/10 text-amber-600 border-amber-600/20", dot: "bg-amber-600" },
  orange: { badge: "bg-orange-600/10 text-orange-600 border-orange-600/20", dot: "bg-orange-600" },
  stone: { badge: "bg-stone-600/10 text-stone-600 border-stone-600/20", dot: "bg-stone-600" },
  red: { badge: "bg-red-600/10 text-red-600 border-red-600/20", dot: "bg-red-600" },
};

function LiveFeedTicker() {
  const doubled = [...feedEvents, ...feedEvents];
  return (
    <div className="relative overflow-hidden border-y border-stone-200/20 py-3 bg-stone-50">
...
function HeroHeadline() {
  ...
  const word = (text: string, visible: boolean, i: number, isAccent = false) => (
    <span
      ...
      className={cn(
        ...
        isAccent &&
          "bg-gradient-to-r from-amber-600 via-orange-500 to-yellow-600 bg-clip-text text-transparent"
      )}
  ...
  return (
    <h1 className="text-5xl sm:text-6xl lg:text-[5.25rem] font-bold leading-[1.08] tracking-[-0.03em] text-stone-900 mb-7">
...
function DashboardMockup() {
  ...
  return (
    <div className="relative rounded-2xl border border-stone-300 bg-stone-100 overflow-hidden shadow-lg">
      {/* Browser chrome */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-stone-300 bg-stone-200/50">
        <div className="flex gap-1.5">
          <div className="h-2.5 w-2.5 rounded-full bg-red-500/70" />
          <div className="h-2.5 w-2.5 rounded-full bg-amber-500/70" />
          <div className="h-2.5 w-2.5 rounded-full bg-green-600/70" />
        </div>
        <div className="flex-1 flex justify-center">
          <div className="bg-stone-300/60 rounded-md px-4 py-1 text-[11px] text-stone-700 flex items-center gap-1.5">
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-4 gap-2.5 mb-4">
          {[
            ...
            <div key={s.label} className="rounded-xl bg-stone-200/60 border border-stone-300 p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] text-stone-600">{s.label}</p>
                <s.icon className={cn("h-3 w-3", s.alert ? "text-red-600" : "text-stone-500")} />
              </div>
              <p className={cn("text-xl font-bold", s.alert ? "text-red-600" : "text-stone-900")}>
                {s.value}
              </p>
            </div>
          ))}
        ...
        {/* Health table */}
        <div className="rounded-xl bg-stone-200/40 border border-stone-300 p-3">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[11px] font-semibold text-stone-700">Integration Health</p>
            ...
            {clients.map((c) => (
              <div key={c.name} className="flex items-center justify-between py-1">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-[12px] text-stone-800">{c.name}</span>
                  <span className="text-[10px] text-stone-600">{c.platform}</span>
                </div>
                <div className="flex items-center gap-2">
                  {c.change !== "-" && (
      ...
      {/* Bottom fade */}
      <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-stone-100 to-transparent pointer-events-none" />
    </div>
  );
}
    ...
    <>
      <motion.nav
        ...
        className={cn(
          ...
          scrolled
            ? "bg-stone-100/80 backdrop-blur-xl border-b border-stone-300/30"
            : "bg-transparent"
        )}
      >
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <span className="text-lg font-bold tracking-tight text-stone-900">
              Corpo<span className="text-amber-700">zort</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link
              href="/signup"
              className="inline-flex items-center gap-1.5 rounded-lg bg-amber-700 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2 transition-all duration-150 active:scale-[0.97]"
            >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 bg-stone-100/95 backdrop-blur-xl border-b border-stone-300/30 px-6 py-6 flex flex-col gap-4 md:hidden"
          >
            {["Features", "How it works", "Pricing"].map((item) => (
              <a
                key={item}
            ...
            <hr className="border-stone-300/30" />
            <Link href="/login" className="text-sm text-stone-600">
              Sign in
            </Link>
            <Link
              href="/signup"
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-700 text-white text-sm font-semibold px-4 py-2.5"
            >
              Get started <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </motion.div>
  ...
  return (
    <div className="space-y-2">
      {faqs.map((faq, i) => (
        <div
          key={i}
          className="rounded-xl border border-stone-300 bg-stone-100/60 overflow-hidden"
        >
          <button
          ...
          >
            ...
            <ChevronDown
              className={cn(
                "h-4 w-4 text-stone-500 transition-transform duration-200 shrink-0 ml-4",
                open === i && "rotate-180"
              )}
            />
          </button>
          <AnimatePresence>
            {open === i && (
              ...
              >
                <p className="px-5 pb-4 text-sm text-stone-700 leading-relaxed">
                  {faq.a}
                </p>
              </motion.div>
            )}
    ...
    <div className="bg-stone-50 text-stone-900">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[60%] w-[900px] h-[900px] rounded-full bg-amber-200/20 blur-[130px]" />
          <div className="absolute top-1/3 right-1/4 w-[400px] h-[400px] rounded-full bg-orange-200/15 blur-[90px]" />
          <div
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 pt-32 pb-20 text-center">
          <motion.div
            ...
            className="inline-flex items-center gap-2 rounded-full bg-stone-200/40 border border-stone-300/50 px-4 py-1.5 mb-10 backdrop-blur-sm"
          >
          ...
          >
            <Link
              href="/signup"
              className="group inline-flex items-center gap-2 rounded-xl bg-amber-700 hover:bg-amber-600 text-white font-semibold px-7 py-3.5 text-base transition-all duration-200 active:scale-[0.97]"
            >
            ...
            <Link
              href="/login"
              className="inline-flex items-center gap-2 rounded-xl bg-stone-200 hover:bg-stone-300 border border-stone-400 text-stone-900 font-medium px-7 py-3.5 text-base transition-all duration-200"
            >
              Sign in
            </Link>
          </motion.div>
          ...
          >
            <div className="absolute -inset-6 bg-amber-200/[0.08] rounded-3xl blur-3xl" />
            <DashboardMockup />
          </motion.div>
        </div>
      </section>
      ...
      <section id="features" className="max-w-6xl mx-auto px-6 py-28">
        <FadeIn className="text-center mb-16">
          <p className="text-xs font-semibold text-amber-700 uppercase tracking-[0.15em] mb-4">
            The problem
          </p>
          <h2 className="text-3xl sm:text-4xl font-bold text-stone-900 mb-4 tracking-tight">
            Agencies lose clients to silent failures
          </h2>
          <p className="text-stone-700 max-w-lg mx-auto text-base">
          ...
          {problems.map((item, i) => {
            ...
            const iconBg =
              item.color === "red"
                ...
                : item.color === "amber"
                ? "bg-amber-600/10 text-amber-700"
                : "bg-stone-600/10 text-stone-700";
            return (
              <FadeIn key={i} delay={i * 0.1}>
                <div
                  className={cn(
                    "rounded-2xl border bg-stone-100/60 p-6 h-full transition-all duration-300 hover:bg-stone-100",
                    borderColor
                  )}
                >
                  {item.tag && (
                    <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-red-700 bg-red-600/10 border border-red-600/20 rounded-full px-2.5 py-0.5 mb-4">
                      {item.tag}
                    </span>
                  )}
                  <div
      ...
      {/* ── SOLUTION ────────────────────────────────────────────────────── */}
      <section className="border-y border-stone-300/40 bg-stone-100/30 py-28">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {solutions.map((item, i) => {
              ...
              return (
                <FadeIn key={i} delay={i * 0.1}>
                  <div className="rounded-2xl border border-stone-300 bg-stone-100/80 p-6 h-full hover:border-stone-400 hover:bg-stone-100 transition-all duration-300">
                    <div
                      className={cn(
                        "rounded-xl w-10 h-10 flex items-center justify-center mb-4",
                        bg
      ...
      <section id="how-it-works" className="max-w-3xl mx-auto px-6 py-28">
        ...
        <div className="relative">
          <div className="absolute left-6 top-10 bottom-10 w-px bg-gradient-to-b from-amber-600/0 via-amber-600/30 to-amber-600/0 hidden sm:block" />
            ...
            {[
              ...
              <FadeIn key={i} delay={i * 0.12}>
                <div className="flex gap-6 relative">
                  <div className="flex-shrink-0 h-12 w-12 rounded-full bg-amber-700 text-white flex items-center justify-center text-sm font-bold z-10 shadow-lg shadow-amber-700/30">
                    {item.step}
                  </div>
      ...
      {/* ── STATS ───────────────────────────────────────────────────────── */}
      <section className="border-y border-amber-600/20 bg-amber-600/[0.06] py-16">
      ...
      <section className="max-w-6xl mx-auto px-6 py-28">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className="rounded-2xl border border-stone-300 bg-stone-100/60 p-6 flex flex-col gap-4 h-full">
                <div className="flex gap-0.5">
                  {Array(5)
                    .fill(0)
                    .map((_, j) => (
                      <span key={j} className="text-amber-700 text-sm">
                        ★
                      </span>
                    ))}
                </div>
                <p className="text-sm text-stone-800 leading-relaxed flex-1 italic">
                  &ldquo;{t.quote}&rdquo;
                </p>
                <div className="flex items-center gap-3 pt-2 border-t border-stone-300/60">
                  <div className="h-8 w-8 rounded-full bg-amber-700/20 border border-amber-700/30 flex items-center justify-center text-[10px] font-bold text-amber-700">
                    {t.avatar}
                  </div>
                  <div>
      ...
      {/* ── PRICING ─────────────────────────────────────────────────────── */}
      <section id="pricing" className="border-y border-stone-300/40 bg-stone-100/30 py-28">
        <div className="max-w-5xl mx-auto px-6">
              ...
              <FadeIn key={plan.name} delay={i * 0.1}>
                <div
                  className={cn(
                    ...
                    plan.highlight
                      ? "border-amber-700/50 bg-amber-600/[0.08] shadow-lg shadow-amber-700/10 ring-1 ring-amber-700/20"
                      : "border-stone-300 bg-stone-100/60 hover:border-stone-400"
                  )}
                >
                  {plan.highlight && (
                    <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-amber-700 bg-amber-700/10 border border-amber-700/20 rounded-full px-2.5 py-0.5 mb-4 w-fit">
                      Most popular
                    </span>
                  )}
                  <p className="text-sm font-medium text-stone-600 mb-1">{plan.name}</p>
                  <p className="text-4xl font-bold text-stone-900 mb-1">
                    ${plan.price}
                    <span className="text-sm font-normal text-stone-600">/mo</span>
                  </p>
                  <p className="text-sm text-stone-700 mb-6">{plan.desc}</p>
                    ...
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-center gap-2.5 text-sm">
                        <CheckCircle
                          className={cn(
                            "h-4 w-4 shrink-0",
                            plan.highlight ? "text-amber-700" : "text-stone-600"
                          )}
                        />
                        <span className="text-stone-800">{f}</span>
                      </li>
                    ))}
                  </ul>
                  <Link
                    ...
                    className={cn(
                      ...
                      plan.highlight
                        ? "bg-amber-700 text-white hover:bg-amber-600 shadow-lg shadow-amber-700/25"
                        : "bg-stone-200 text-stone-900 hover:bg-stone-300 border border-stone-400"
                    )}
                  >
                    {plan.cta}
                  </Link>
          ...
          <FadeIn className="mt-8 text-center">
            <p className="text-sm text-stone-700 flex items-center justify-center gap-2">
      ...
      {/* ── FINAL CTA ───────────────────────────────────────────────────── */}
      <section className="relative py-32 overflow-hidden border-t border-stone-300/40">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full bg-amber-200/15 blur-[120px]" />
          <div
            className="absolute inset-0 opacity-[0.02]"
            style={{
              backgroundImage: `linear-gradient(rgba(0,0,0,0.03) 1px, transparent 1px),
        ...
        <FadeIn className="relative max-w-3xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 rounded-full bg-stone-200/40 border border-stone-300/50 px-4 py-1.5 mb-8">
            <span className="text-xs text-stone-700">
              Join agencies already monitoring
            </span>
          </div>

          <h2 className="text-4xl sm:text-5xl font-bold text-stone-900 mb-5 tracking-tight">
          ...
          <p className="text-stone-700 mb-10 text-lg max-w-md mx-auto">
            Set up in 2 minutes. Free up to 3 clients. No credit card required.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/signup"
              className="group inline-flex items-center gap-2 rounded-xl bg-amber-700 hover:bg-amber-600 text-white font-semibold px-8 py-4 text-base transition-all duration-200 shadow-lg shadow-amber-700/20"
            >
              Start monitoring free
              <ArrowRight className="h-4 w-4 group-hover:translate-x-0.5 transition-transform" />
            </Link>
            <Link
              href="/login"
              className="inline-flex items-center gap-2 rounded-xl bg-stone-200 hover:bg-stone-300 border border-stone-400 text-stone-900 font-medium px-8 py-4 text-base transition-all duration-200"
            >
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer className="border-t border-stone-300/40 py-10">
        <div className="max-w-6xl mx-auto px-6">
